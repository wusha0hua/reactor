...
Connection::Connection(const std::unique_ptr<EventLoop>& loop, std::unique_ptr<Socket> client_socket): loop(loop), client_socket(std::move(client_socket)), connection_channel(new Channel(this->client_socket->fd(), loop->get_epoll())) {
	is_disconnect = false;
	//connection_channel = new Channel(this->client_socket->fd(), loop->get_epoll());
