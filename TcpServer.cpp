...
TcpServer::TcpServer(std::string ip, uint16_t port, size_t thread_num): main_loop(new EventLoop()), acceptor(Acceptor(main_loop, ip, port))/*, threadpool(thread_num)*/ {
...
void TcpServer::accept_connection(std::unique_ptr<Socket> client_socket) {
	SPConnection conn(new Connection(main_loop, std::move(client_socket)));
	...
	std::cout << "client socket: " << conn->get_fd() << std::endl;
